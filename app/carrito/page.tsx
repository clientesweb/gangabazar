      ...
      <div className="min-h-screen">
        ...
        <main className="flex min-h-[60vh] items-center justify-center px-4">
          <div className="text-center">
            ...
            <h1 className="mt-6 font-serif text-2xl font-light text-foreground">Tu carrito esta vacio</h1>
            <p className="mt-2 text-sm text-muted-foreground">Descubri productos personalizados con grabado laser.</p>
    ...
    <div className="min-h-screen">
      ...
      <main className="bg-background px-4 py-12 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          ...
          <h1 className="font-serif text-3xl font-light text-foreground sm:text-4xl">Tu carrito</h1>
          ...
          <div className="mt-10 grid gap-10 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2">
              <div className="divide-y divide-border">
                {items.map((item) => (
                  <div key={`${item.product.id}-${item.size}-${item.color}`} className="flex gap-4 py-6 sm:gap-6">
                    <Link href={`/producto/${generateSlug(item.product.name)}`} className="flex-shrink-0">
                    ...
                    <div className="flex flex-1 flex-col">
                      <div className="flex justify-between">
                        <div>
                          <Link href={`/producto/${generateSlug(item.product.name)}`}>
                      ...
                      <div className="mt-auto flex items-end justify-between">
                        ...
                        <p className="text-base font-medium text-foreground">${item.product.price * item.quantity}</p>
                      </div>
                    </div>
                  </div>
                ))}
