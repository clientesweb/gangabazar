...
import { generateGrabadoSlug } from "@/lib/services/grabados"

interface GrabadoDetailProps {
  grabado: any
  relatedGrabados?: any[]
}

export function GrabadoDetailClient({ grabado, relatedGrabados = [] }: GrabadoDetailProps) {
  const { addItem } = useCart()

  // Generate slug from grabado name
  const slug = generateGrabadoSlug(grabado.name)

  // Setup images - handle both old and new schema
  const images = grabado.images && grabado.images.length > 0
    ? grabado.images.map((img: any) => typeof img === 'string' ? img : img.image_url)
    : [grabado.mainImage || '/placeholder.svg']
  const [selectedImage, setSelectedImage] = useState(0)

  // Setup variants/colors
  const variants = grabado.variants || []
  ...
  const handleAddToCart = () => {
    ...
    addItem(grabado, "Único", colorName, quantity)
    setAddedToCart(true)
      ...
      <div className="bg-background px-4 py-4 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <nav className="flex items-center gap-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <span className="text-foreground">{grabado.name}</span>
          </nav>
        </div>
      </div>
          ...
          <div className="grid gap-8 lg:grid-cols-2 lg:gap-16">
              ...
              <div className="flex gap-2 sm:flex-col">
                {images.map((image: string, index: number) => (
                  ...
                  >
                    <img
                      src={image}
                      alt={`${grabado.name} - Vista ${index + 1}`}
            ...
            <div className="flex flex-col">
              <span className="text-xs uppercase tracking-widest text-muted-foreground">
                {grabado.tagline || slug || "Grabado"}
              </span>
              <h1 className="mt-2 font-serif text-3xl font-light text-foreground sm:text-4xl">{grabado.name}</h1>
              <p className="mt-2 text-lg text-foreground">${grabado.price.toLocaleString()}</p>
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{grabado.description}</p>
              ...
              <Accordion type="single" collapsible className="mt-8 border-t border-border">
                <AccordionItem value="details">
                  ...
                  <AccordionContent>
                    <p className="text-sm text-muted-foreground">{grabado.description}</p>
                  </AccordionContent>
                </AccordionItem>
